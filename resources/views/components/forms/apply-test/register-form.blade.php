<form class="flex flex-col w-full" method="POST" id="form-search" autocomplete="off" action="{{ $route }}">
    ...
    <input name="period_num_to" type="text" class="form-control" value="{{ request()->query('period_num_to') }}" hidden>
    <input name="period_num_from" type="text" class="form-control" value="{{ request()->query('period_num_from') }}" hidden>
    <div class="card-body">
        <table class="table table-bordered table-view">
            <tbody>
                <tr>
                    ...
                    <td>
                        <input name="la_type" class="form-control" value="{{ old('la_type', $data->la_type) }}" disabled>
                    </td>
                </tr>
                <tr>
                    <th class="w-20">検定日</th>
                    <td>
                        <input name="test_date" class="form-control" type="date" value="{{ old('test_date', $data->test_date) }}" disabled>
                    </td>
                </tr>
                <tr>
                    <th class="w-20">実施回</th>
                    <td>
                        <input name="num_of_days" type="text" class="form-control" value="{{ old('num_of_days', $data->num_of_days) }}" disabled>
                    </td>
                </tr>
                <tr>
                    <th class="w-20">実施時限</th>
                    <td>
                        <input name="period_num" type="text" class="form-control" value="{{ $data->period_name_from. '～'. $data->period_name_to }}" disabled>
                    </td>
                </tr>
            </tbody>
        </table>
                ...
                <div class="col-sm-12">
                    <table class="table table-bordered table-hover dataTable dtr-inline">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><input type="checkbox" name="ledger_id[]" value="{{ $item->admCheckItem->first()->ledger_id }}"></td>
    ...
    <div class="card-footer">
        <div class="col text-center">
            <a href="{{ route('apply-test.index')}}" class="btn btn-sm btn-secondary">キャンセル</a>
            <button type="submit" class="btn btn-sm btn-primary ml-1">保存</button>
        </div>
    </div>
</form>